...
if __name__ == '__main__':
    ...
    for pair_sentence in pair_sentences:
        for chunks_pair in pair_sentence:
            if is_NV(chunks_pair):
                res_NVs.append(chunks_pair)
    for res_NV in res_NVs[:10]:
        noun = ''.join([morph.surface for morph in res_NV[0]])
        verb = ''.join([morph.surface for morph in res_NV[1]])
